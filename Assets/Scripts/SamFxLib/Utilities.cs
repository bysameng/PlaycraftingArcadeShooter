...
//			colors[i] = new Color(Random.value, Random.value, Random.value, 1f);
	...
	public static Color SetSaturation(this Color color, float amount){
		HSBColor hsb = new HSBColor(color);
		hsb.s = amount;
		return hsb.ToColor();
	}
	public static Color SetSB(this Color color, float s, float b){
		HSBColor hsb = new HSBColor(color);
		hsb.b = b;
		hsb.s = s;
		return hsb.ToColor();
